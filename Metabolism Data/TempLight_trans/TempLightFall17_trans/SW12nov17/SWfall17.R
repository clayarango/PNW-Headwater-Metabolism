setwd("N:/Thesis.N/FreshStart_trans/TempLight_trans/TempLightFall17_trans/SW12nov17")
install.packages("chron")
library(chron)
StreamData=read.table("SW12nov17.csv",header=TRUE)  	##time is in seconds since 1970 and in UTC
StreamData$dtime<-chron(StreamData$time/86400)-(8/24)
...
plot(StreamData$dtime, StreamData$oxy,cex.lab=1.5, cex.axis=1.5, lwd=1,ylim=c(0,12), xlab="Time",ylab="Dissolved O2 (mg/L)")
...
nightreg(StreamData[StreamData$dtime>=as.numeric(chron(dates="11/08/17", times="00:00:00")) & StreamData$dtime<=as.numeric(chron(dates="11/08/17", times="23:59:59")), ], bp=660, ts=10)
...
# 5 MIN timestep!! iterature k600 values based on slope
rivermetab(o2file=StreamData[StreamData$dtime>=as.numeric(chron(
  dates="11/12/17", times="15:25:00")) & StreamData $dtime<=as.numeric(chron(
    dates="11/14/17", times="11:05:00")), ], 
  z=0.060, 
  bp=710, 
  ts=0.0034722, 
  K=99.966963)
# 10 MIN timestep!! iterature k600 values based on slope
rivermetab(o2file=StreamData[StreamData$dtime>=as.numeric(chron(
  dates="11/12/17", times="15:25:00")) & StreamData $dtime<=as.numeric(chron(
    dates="11/14/17", times="11:05:00")), ], 
  z=0.060, 
...
# 5 MIN timestep!! my derived k600 values based on velocity
rivermetab(o2file=StreamData[StreamData$dtime>=as.numeric(chron(
  dates="11/12/17", times="15:25:00")) & StreamData $dtime<=as.numeric(chron(
  dates="11/14/17", times="11:05:00")), ], 
  z=0.060, 
...
# 5 MIN timestep!! full model as is 
